...
# Test script for Type 1 (세, 새, 가, 나, 대, 배, 사, 이)
...
<body>
<h2>Type 1 세로모음 비례 및 간격 검증 (세, 새, 가, 내, 대, 사)</h2>
<div class="row" id="row"></div>
<script>
const chars = ['세', '새', '가', '내', '대', '사'];
...
with open('test/preview_type1.html', 'w', encoding='utf-8') as f:
    f.write(test_html)

edge_path = r'C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe'
url = 'file:///' + os.path.abspath('test/preview_type1.html').replace('\\', '/')
p1 = os.path.abspath('test/type1_current.png')
subprocess.run([edge_path, '--headless', '--disable-gpu', f'--screenshot={p1}', '--window-size=950,550', url])
print('Done capture type1 current!')
