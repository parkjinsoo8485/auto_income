...
# Standard Stroke Database (0~100 normalized)
STROKE_DB = {
    'ㄱ': [
    ...
    'ㅎ': [
      {'d': 'M 36,12 L 64,12', 'desc': '1획: 꼭지 점획'},
      {'d': 'M 18,28 L 84,28', 'desc': '2획: 가로'},
      {'d': 'M 50,42 C 34,42 22,54 22,68 C 22,82 34,92 50,92 C 66,92 78,82 78,68 C 78,54 66,42 50,42 Z', 'desc': '3획: 원'}
    ],

    # 모음
    'ㅏ': [
      {'d': 'M 38,15 L 38,85', 'desc': '1획: 세로'},
      {'d': 'M 38,48 L 82,48', 'desc': '2획: 가로'}
    ],
    'ㅐ': [
      {'d': 'M 32,15 L 32,85', 'desc': '1획: 왼 세로'},
      {'d': 'M 32,48 L 68,48', 'desc': '2획: 중간 가로'},
      {'d': 'M 68,15 L 68,85', 'desc': '3획: 오른 세로'}
    ],
    'ㅑ': [
      {'d': 'M 38,15 L 38,85', 'desc': '1획: 세로'},
      {'d': 'M 38,36 L 82,36', 'desc': '2획: 위 가로'},
      {'d': 'M 38,60 L 82,60', 'desc': '3획: 아래 가로'}
    ],
    'ㅒ': [
      {'d': 'M 32,15 L 32,85', 'desc': '1획: 왼 세로'},
      {'d': 'M 32,36 L 68,36', 'desc': '2획: 위 가로'},
      {'d': 'M 32,60 L 68,60', 'desc': '3획: 아래 가로'},
      {'d': 'M 68,15 L 68,85', 'desc': '4획: 오른 세로'}
    ],
    'ㅓ': [
      {'d': 'M 18,48 L 62,48', 'desc': '1획: 가로'},
      {'d': 'M 62,15 L 62,85', 'desc': '2획: 세로'}
    ],
    'ㅔ': [
      {'d': 'M 28,48 L 60,48', 'desc': '1획: 가로'},
      {'d': 'M 60,15 L 60,85', 'desc': '2획: 왼 세로'},
      {'d': 'M 82,15 L 82,85', 'desc': '3획: 오른 세로'}
    ],
    'ㅕ': [
      {'d': 'M 18,36 L 62,36', 'desc': '1획: 위 가로'},
      {'d': 'M 18,60 L 62,60', 'desc': '2획: 아래 가로'},
      {'d': 'M 62,15 L 62,85', 'desc': '3획: 세로'}
    ],
    'ㅖ': [
      {'d': 'M 28,36 L 60,36', 'desc': '1획: 위 가로'},
      {'d': 'M 28,60 L 60,60', 'desc': '2획: 아래 가로'},
      {'d': 'M 60,15 L 60,85', 'desc': '3획: 왼 세로'},
      {'d': 'M 82,15 L 82,85', 'desc': '4획: 오른 세로'}
    ],
    'ㅗ': [
...
def get_slots(dec):
    ...
    if isVert:
        if not hasJong:
            ...
            # 초성과 중성 간격 최적화 (초성 w: 74, 중성 x: 104, w: 70)
            slots.append({'jamo': cho, 'role': 'cho', 'x': 26, 'y': 25, 'w': 74, 'h': 146, 'type': 1})
            slots.append({'jamo': jung, 'role': 'jung', 'x': 104, 'y': 22, 'w': 70, 'h': 154, 'type': 1})
        else:
            # Type 4: 받침 있는 세로 모음 (강, 날, 달, 한, 닭)
            slots.append({'jamo': cho, 'role': 'cho', 'x': 26, 'y': 20, 'w': 72, 'h': 76, 'type': 4})
            slots.append({'jamo': jung, 'role': 'jung', 'x': 102, 'y': 18, 'w': 70, 'h': 82, 'type': 4})
...
for word in test_words:
    ...
    for s in slots:
        ...
        if glyph:
            ...
            g_path = glyph['path']
            underlay_paths.append(f'<path d="{g_path}" transform="{t}" fill="rgba(255,255,255,0.15)" />')
        ...
        for sub_idx, j in enumerate(jamo_list):
            ...
            for st in raw_strokes:
                ...
                stroke_paths.append(f'<path d="{t_path}" stroke="{col}" stroke-width="12" stroke-linecap="round" stroke-linejoin="round" fill="none" opacity="0.88"/>')
...
<body>
<h1>한글 6대 결합규칙 + 회색 폰트 벡터 밑그림 + 획순 정렬 테스트</h1>
{''.join(html_cards)}
</body>
</html>
'''

with open('test/test_strokes_with_underlay.html', 'w', encoding='utf-8') as f:
    f.write(full_html)

edge_path = r"C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe"
screenshot_path = os.path.abspath("test/screenshot_strokes_underlay.png")
html_url = "file:///" + os.path.abspath("test/test_strokes_with_underlay.html").replace("\\", "/")
