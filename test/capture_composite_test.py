...
# Test script for composite vowels (월, 원, 권, 화, 환, 귀)
...
<body>
<h2>복합모음 비례 검증 (월, 원, 권, 화, 환, 귀)</h2>
<div class="row" id="row"></div>
<script>
const chars = ['월', '원', '권', '화', '환', '귀'];
...
with open('test/preview_composite.html', 'w', encoding='utf-8') as f:
    f.write(test_html)

edge_path = r'C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe'
url = 'file:///' + os.path.abspath('test/preview_composite.html').replace('\\', '/')
p1 = os.path.abspath('test/composite_current.png')
subprocess.run([edge_path, '--headless', '--disable-gpu', f'--screenshot={p1}', '--window-size=950,550', url])
print('Done capture composite current!')
