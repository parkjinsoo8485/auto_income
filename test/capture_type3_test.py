...
# Test script for Type 3 (화, 과, 귀, 의, 왜)
...
<body>
<h2>Type 3 화, 과, 귀 결합 거리 검증</h2>
<div class="row" id="row"></div>
<script>
const chars = ['화', '과', '귀', '의', '왜', '원'];
...
with open('test/preview_type3.html', 'w', encoding='utf-8') as f:
    f.write(test_html)

edge_path = r'C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe'
url = 'file:///' + os.path.abspath('test/preview_type3.html').replace('\\', '/')
p1 = os.path.abspath('test/type3_current.png')
subprocess.run([edge_path, '--headless', '--disable-gpu', f'--screenshot={p1}', '--window-size=950,550', url])
print('Done capture type3 current!')
