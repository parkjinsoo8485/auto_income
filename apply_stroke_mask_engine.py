...
CSS_TARGET_OLD = re.compile(
    r'\.stroke-view-container\{padding:20px;display:flex;flex-direction:column;align-items:center;min-height:55vh\}'
    r'[\s\S]*?@keyframes drawHangul\{0%\{stroke-dashoffset:1200;fill:transparent\}60%\{stroke-dashoffset:0;fill:transparent\}80%,100%\{stroke-dashoffset:0;fill:rgba\(99,102,241,0\.25\)\}\}'
)

CSS_REPLACEMENT = """/* ─── STROKE ORDER (획순 마스킹 엔진) ─── */
...
@keyframes showStrokeMarker{to{opacity:1}}"""

JS_TARGET_OLD = re.compile(
    r'// ─── Stroke Order \(✍️ 획순 따라쓰기\) Mode ───[\s\S]*?'
    r'function prevStroke\(\)\{ const words=TOPIK_DATA\[S\.level\]\|\|\[\]; S\.strokeIndex=\(S\.strokeIndex-1\+words\.length\)%words\.length; renderStrokeMode\(\); \}'
)

JS_REPLACEMENT = r"""// ══════════════════════════════════════════════════════════════════
//  한글 획순 마스킹 엔진 (Hangul Stroke Masking & Reveal Engine)
//  - 국립국어원 표준 획순 골격 DB + SVG Mask를 통한 폰트 글리프 브러시 채움
//  - 음절 6대 레이아웃 분해 + 인터랙티브 글자 탭 / 컨트롤 / 직접 쓰기
// ══════════════════════════════════════════════════════════════════
...
// 국립국어원 표준 획순 골격선 데이터베이스 (정규화 0~100)
const HANGUL_STROKE_DB = {
  ...
  'ㅇ': [
    { d: 'M 50,18 A 32,32 0 1,0 49.9,18 Z', desc: '1획: 반시계 방향 원' }
  ],
  'ㅈ': [
...
// 6대 음절꼴 레이아웃 계산 (viewBox 0 0 200 200 기준)
...
function renderStrokeMode(){
  ...
  strokes.forEach((st, idx) => {
    ...
    if (S.strokeShowNumbers) {
      ...
      const bx = nums[0] || 100;
      const by = nums[1] || 100;
      const col = markerColors[idx % markerColors.length];
      numberMarkers += `
        <g style="opacity:0; animation: showStrokeMarker 0.25s ease ${delay.toFixed(2)}s forwards;">
          <circle cx="${bx}" cy="${by}" r="8.5" fill="var(--card)" stroke="${col}" stroke-width="2"/>
          <text x="${bx}" y="${by}" fill="${col}" font-size="9.5" font-weight="900" text-anchor="middle" dominant-baseline="central" font-family="'Inter', sans-serif">${idx+1}</text>
        </g>
      `;
    }
  });
  ...
  const svgHTML = `
    <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- 1. 밑그림 가이드 폰트 (연한 회색 고품질 Noto 글씨) -->
      <text x="100" y="105"
...
def patch_file(filepath):
    ...
    # 1. CSS 교체 (이미 패치되었을 수도 있으므로 체크)
    if CSS_TARGET_OLD.search(content):
        content = CSS_TARGET_OLD.sub(lambda m: CSS_REPLACEMENT, content, count=1)
        print(f"  [OK] CSS patched in {filepath}")
    elif "stroke-canvas-overlay" in content:
        print(f"  [INFO] CSS already patched in {filepath}")
    else:
        print(f"  [WARN] CSS target not found in {filepath}")

    # 2. JS 함수 교체
    if JS_TARGET_OLD.search(content):
        content = JS_TARGET_OLD.sub(lambda m: JS_REPLACEMENT, content, count=1)
        print(f"  [OK] JS stroke engine patched in {filepath}")
    elif "decomposeHangulSyllable" in content:
        print(f"  [INFO] JS stroke engine already patched in {filepath}")
    else:
